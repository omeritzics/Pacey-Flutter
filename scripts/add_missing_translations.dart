  ...
  static Future<String?> translate(String text, String targetLanguage) async {
    try {
      final response = await http.get(
        Uri.parse('$_baseUrl?client=gtx&sl=en&tl=$targetLanguage&dt=t&q=${Uri.encodeComponent(text)}'),
      );
...
void main(List<String> args) async {
  ...
  for (final targetFile in arbFiles) {
    ...
    final languageCode = targetFilename.replaceAll(RegExp(r'^app_(.+)\.arb$'), r'\1');
      ...
      if (!targetData.containsKey(key)) {
        ...
        if (key.startsWith('@')) {
          ...
          if (autoTranslate && value is String && value.isNotEmpty) {
            final translated = await AutoTranslator.translate(value, languageCode);
            if (translated != null) {
